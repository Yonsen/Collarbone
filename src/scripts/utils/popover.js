...
define("utils/popover", ["utils/base", "utils/dom", "utils/drag", "utils/mediator"], function(Base, DOM, Drag, Mediator){
    ...
    function Popover(params) {
        ...
        var btns = options.ok == '' ? '' : '<a id="' + UUID + '_ok" class="col ok btn btn-primary" href="javascript:;">' + options.ok + '</a>';
        btns += options.no == '' ? '' : '<a id="' + UUID + '_no" class="col no btn btn-default" href="javascript:;">' + options.no + '</a>';
        var btn = options.close ? '<a id="' + UUID + '_close" class="close" href="javascript:;" title="关闭">x</a>' : '';
        ...
        html += btns == '' ? '' : '<div id="' + UUID + '_foot" class="popup_foot row">' + btns + '</div>';
